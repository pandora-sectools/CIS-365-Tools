# 5.2.2.4 Ensure Sign-in frequency is enabled and browser sessions are not persistent for Administrative users (Automated)
# E3 Level 1
# E5 Level 1
...
foreach ($Policy in $Policies) {
    ...
    if (
        $SignInFrequency.isEnabled -eq $true -and
    ...
    elseif (
        $SignInFrequency.isEnabled -eq $true -and
        $SignInFrequency.frequencyInterval -eq "timeBased" -and
        $SignInFrequency.type -eq "hours" -and
        $SignInFrequency.value -le 4
    ) {
        $ValidFrequency = $true
    }
    ...
    $Obj = [PSCustomObject]@{
        ...
        PersistentBrowserEnabled     = $PersistentBrowser.isEnabled
        PersistentBrowserMode        = $PersistentBrowser.mode
    ...
    if ($ValidFrequency -ne $true) { $Obj.AuditState = "FAIL" }

    if ($Obj.PersistentBrowserEnabled -ne $true) { $Obj.AuditState = "FAIL" }

    if ($Obj.PersistentBrowserMode -ne "never") { $Obj.AuditState = "FAIL" }
...
if (@($PassingPolicies).Count -gt 0) {
    Write-Host "** PASS : Found a compliant Conditional Access session policy for administrative users. **"
    $PassingPolicies | Format-List
} else {
    Write-Host "** FAIL : No qualifying Conditional Access session policy for administrative users was found. **"
